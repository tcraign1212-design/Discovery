...
def generate_txt(content_text):
    """Generates a clean plaintext fallback for instant exporting."""
    buffer = io.BytesIO()
    buffer.write(content_text.encode('utf-8'))
    buffer.seek(0)
    return buffer
...
with col2:
    ...
    if "last_analysis_output" in st.session_state:
        ...
        with c2:
            txt_data = generate_txt(output_text)
            st.download_button(
                label="📥 Download as Text File (.txt)",
                data=txt_data,
                file_name="Targeted_Discovery_Package.txt",
                mime="text/plain",
                use_container_width=True
            )
