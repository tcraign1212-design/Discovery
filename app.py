...
def build_prompt(stage, ctype, dlevel, doi, sol, summary, gov, cv_jurisdiction):
    ...
    cv_txt = f"\n- {ctype.upper()} FLAG: Apply {cv_jurisdiction} analysis." if ctype in ["Trucking", "Commercial Vehicle"] else ""
...
with col1:
    ...
    cv_jurisdiction = st.radio("FMCSR Scope:", ["Interstate (Federal)", "Intrastate (Texas)", "Unsure"], index=2, horizontal=True) if case_type in ["Trucking", "Commercial Vehicle"] else "N/A"
...
with col2:
    ...
    if run_brief:
        if not case_summary.strip() or not active_key:
            st.warning("Veto: Missing summary or API key.")
        else:
