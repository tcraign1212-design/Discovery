...
def build_prompt(stage, ctype, dlevel, doi, sol, summary, gov, cv_jurisdiction):
    ...
    cv_txt = f"\n- {ctype.upper()} FLAG: Apply {cv_jurisdiction} analysis for driver logs/DQF." if ctype in ["Trucking", "Commercial Vehicle"] else ""
    
    params = f"Framework: {ctype}\nDOI: {doi}\nSOL: {sol}\nGov Entity: {gov}\n\nSummary:\n{summary}"
    
    if stage == "Pre-Litigation":
        return f"Draft Texas Pre-Suit Brief. {params} {gov_txt}{cv_txt} Use Headers: ## 1. Chronology, ## 2. Liability, ## 3. Risk Flags, ## 4. Proof Gaps, ## 5. Defense Anticipation, ## 6. Action Items."
    return f"Draft Texas Litigation Blueprint. {params} Discovery: {dlevel} {gov_txt}{cv_txt} Use Headers: ## 1. Chronology, ## 2. Liability, ## 3. Proof Gaps, ## 4. Defense Anticipation, ## 5. Discovery Blueprint, ## 6. Strategic Flags."
...
with col1:
    ...
    st.markdown("---")
    case_type = st.selectbox("Framework:", ["Standard MVA", "Commercial Vehicle", "Trucking", "Premises", "Workplace", "UM/UIM", "TTCA"])
...
with col2:
    ...
    if run_brief:
        if not case_summary.strip() or not active_key:
            st.warning("Veto: Missing data or API key.")
        else:
